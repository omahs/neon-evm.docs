...
_Neon EVM is a low-friction solution to scale Ethereum dApps on Solana._

<ImgTheme
  src='/img/doc-images/about/infographic.svg'
  srcDark='/img/doc-images/about/infographic_dark.svg'
/>
...
Neon EVM makes the key Ethereum dApp tools compatible with Solana, including Vyper, Solidity, [MetaMask](/docs/wallet/metamask_setup), [Hardhat](/docs/developing/deploy_facilities/using_hardhat), [Remix](/docs/developing/deploy_facilities/using_remix) and [Foundry](to be included). The solution allows any Ethereum application to run on Solana with minimal reconfiguration, this includes Uniswap, SushiSwap, 0x, and MakerDAO.

### Who uses Neon EVM?

Teams interested in significant savings (thanks to the low gas fees, or scaling with the high throughput that Solana provides) are likely to be the first movers. Neon EVM serves any developers or product teams who wish that their dApp could reach customers on Solana. Neon EVM also opens up the liquidity on Solana to DeFi products.
...
- Well-known Ethereum tools: MetaMask, Hardhat, Remix, Foundry, etc.
